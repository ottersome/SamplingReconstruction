...
def sinc_interpolation(padded_signal,upsamp_factor,fc=None, num_lobes=10):
    ...
    lint = np.arange(-upsamp_factor*N//2,upsamp_factor*N//2)/float(upsamp_factor)# To fill the kernel.
...
def sinc_interp(x,s,u):
    finer_u = np.outer(u, 1/s)
    indices = np.outer(np.ones_like(u), np.arange(len(x))) # Broadcast horizontally(Displacement)
    displacement = finer_u-indices
    # Convolve
